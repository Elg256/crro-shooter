...
def load_images():
    # Charger les images associées à chaque arme
    asval_image = pygame.image.load("player/0.png")
    awp_image = pygame.image.load("player/awp.png")
    mp5_image = pygame.image.load("player/mp5.png")

    return [asval_image, awp_image, mp5_image]


def choose_weapon():
    weapons = ["asval", "awp", "mp5"]  # Ajoutez ici les choix d'armes

    # Créer une fenêtre Pygame
    weapon_screen = pygame.display.set_mode((0, 0), pygame.RESIZABLE)
    pygame.display.set_caption('Appuyez sur 1, 2 ou 3 :')

    font = pygame.font.Font("font/PublicPixel.ttf", 30)

    images = load_images()  # Charger les images associées aux armes

    while True:
        weapon_screen.fill((97, 92, 95))

        text = font.render('Appuyez sur 1, 2 ou 3 :', True, (0, 0, 0))
        weapon_screen.blit(text, (50, 50))

        textinfo = font.render('Press esc during game to leave.', True, (0, 0, 0))
        weapon_screen.blit(textinfo, (250, 500))

        for i, weapon in enumerate(weapons):
            num = str(i + 1)
            weaponshow = num + " " + weapon
            text = font.render(weaponshow, True, (0, 0, 0))
            weapon_screen.blit(text, (50, 150 + i * 85))

            # Afficher l'image associée à chaque arme
            weapon_screen.blit(images[i], (200, 25 + i * 85))
...
# Charger l'image du joueur principal (0.png)
if armes == "asval":
    player_image = pygame.image.load("player/awp.png").convert_alpha()
elif armes == "awp":
    player_image = pygame.image.load("player/awp.png").convert_alpha()
else:
    player_image = pygame.image.load("player/awp.png").convert_alpha()
...
player_image = pygame.transform.rotozoom(player_image, 0, 0.35)
player_image_rect = player_image.get_rect(center=(WIDTH // 2, HEIGHT // 2))

# Charger l'image du joueur supplémentaire (player01.png)
additional_player_image = pygame.image.load("player/player.png").convert_alpha()
additional_player_image_rect = additional_player_image.get_rect(center=(WIDTH // 2, HEIGHT // 2))
...
if armes == "asval":
    gun_shot_sound = pygame.mixer.Sound("sounds/asvalshot.mp3")
    gun_shot_sound.set_volume(0.2)
elif armes == "awp":
    gun_shot_sound = pygame.mixer.Sound("sounds/awpshot.mp3")
    gun_shot_sound.set_volume(0.5)
else:
    gun_shot_sound = pygame.mixer.Sound("sounds/gunshot.wav")
    gun_shot_sound.set_volume(0.1)
...
nightborne_death_sound.set_volume(0.2)
necromancer_death_sound.set_volume(0.2)
...
class Player(pygame.sprite.Sprite):
    def __init__(self, pos):
        ...
        if armes == "asval":
            ...
            self.image = pygame.image.load("player/awp.png").convert_alpha()
        else:
            self.image = pygame.image.load("player/mp5.png").convert_alpha()
...
class Enemy(pygame.sprite.Sprite): 
    def __init__(self, name, position):
        ...
        self.notice_radius = random.choice(enemy_info["notice_radius"])
    ...
    def update(self):
        ...
        if self.alive:
            ...
            if self.get_vector_distance(pygame.math.Vector2(player.base_player_rect.center), 
                ...
                self.roam()
                if self.get_vector_distance(pygame.math.Vector2(player.base_player_rect.center), pygame.math.Vector2(self.base_zombie_rect.center)) < 2000:
...
class GameLevel(pygame.sprite.Group): 
    ...
    def custom_draw(self): 
        ...
        screen.blit(additional_player_image, additional_player_image_rect)
...
necromancer_image = pygame.transform.rotozoom(necromancer_image ,3, 3)
...
nightborne_image = pygame.transform.rotozoom(nightborne_image,3,3)
...
def display_end_screen():
    ...
    if armes == "asval":
        ...
        image_rect = image.get_rect(center=(1200, 500))
        weaponsuse = font.render(f"Weapon use:", False, WHITE)
        screen.blit(weaponsuse, (1150, 450))
        screen.blit(image,image_rect)
    elif armes == "awp":
        image = pygame.image.load("player/awp.png").convert_alpha()
        image_rect = image.get_rect(center=(1200, 500))
        weaponsuse = font.render(f"Weapon use:", False, WHITE)
        screen.blit(weaponsuse, (1150, 450))
        screen.blit(image,image_rect)
    else:
        image = pygame.image.load("player/mp5.png").convert_alpha()
        image_rect = image.get_rect(center=(1200, 500))
        weaponsuse = font.render(f"Weapon use:", False, WHITE)
        screen.blit(weaponsuse, (1150, 450))
        screen.blit(image,image_rect)
    ...
    score_text_4 = font.render(f"Wave:", False, WHITE)
    score_text_5 = score_font.render(f"{game_stats['current_wave']}", False, WHITE)
    screen.blit(score_text_4, (550, 350))
    screen.blit(score_text_5, (550, 450))
...
while True: 
    ...
    for event in pygame.event.get():
        ...
        if not game_active and keys[pygame.K_p]:
            ...
            if event.key == pygame.K_ESCAPE:
                # Quitter si la touche Escape est enfoncée
                pygame.quit()
    ...
    if game_active:
        ...
        if game_stats['enemies_killed_or_removed'] == game_stats["number_of_enemies"][game_stats["current_wave"] - 1]: # level over  
            ...
            waves = game_stats["current_wave"]
            print(waves)

            # Ouvrir le fichier en mode écriture
            with open('mon_fichier.txt', 'w') as fichier:
                # Écrire un entier dans le fichier
                entier_a_enregistrer = waves
                fichier.write(str(entier_a_enregistrer))
